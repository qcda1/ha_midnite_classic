...
Allez sur [https://github.com/qcda1/ha_midnite_classic/releases](https://github.com/qcda1/ha_midnite_classic/releases)
Cliquez sur "Create a new release"
Sélectionnez le tag existant ou créez-en un nouveau
Remplissez le formulaire (voir section suivante)
📝 Exemple concret avec release notes
...
Attendez jusqu'à 24h (HACS ne vérifie pas en temps réel)
Forcez la vérification : HACS → ⋮ → "Redémarrer HACS"
Vérifiez que le tag existe sur GitHub : git ls-remote --tags origin
Assurez-vous que le tag correspond EXACTEMENT à la version dans manifest.json
Problème : La release notes n'apparaît pas

Solutions :

Vérifiez que vous avez bien rempli la description de la release
La release doit être "publiée" (pas en "draft" ou "pre-release")
Attendez quelques minutes après publication
...
Testez toujours localement avant de créer une release
Rédigez des release notes claires même pour les petites mises à jour
Utilisez des préfixes dans vos commits (🐛 Fix:, ✨ Feat:, 📝 Docs:)
Gardez un CHANGELOG.md dans votre dépôt pour l'historique
Communiquez les breaking changes en MAJUSCULES dans les release notes
